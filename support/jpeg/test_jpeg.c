...
/*****
      test_jpeg.c -
      Test program for the JPEG library.  Reads a JPEG file and pixel
      ...
      Call:
        test_jpeg <JPEG image> <output file> <x> <y>
...
void usage(void) {
  ...
  printf("Usage:  test_jpeg <image> <output (changed pixel)> <x> <y>\n");
  printf("  exiting ...\n");
  exit(1);
}
